...
def initalizeCentroids(dataset, k):
  ...
  randCs = np.random.choice(dataset.shape[0], size=k, replace=False)
  centroids = dataset[randCs]
...
def computeAssignments(dataset, centroids):
  ...
  for i in range(distances.shape[0]):
    assignments[i,0] = np.argmin(distances[i])
...
def updateCentroids(dataset, centroids, assignments):
    ...
    new_centroids = np.zeros((centroids.shape[0],dataset.shape[1]))

    for i in range(dataset.shape[0]):
      centroidIdx = int(assignments[i,0])
      counts[centroidIdx] += 1
      new_centroids[centroidIdx] += dataset[i]

    for k in range(new_centroids.shape[0]):
...
def calculateSSE(dataset, centroids, assignments):
    ...
    for i in range(dataset.shape[0]):
        ...
        sse += np.sum((dataset[i] - centroids[centroidIdx]) ** 2)
    return sse
